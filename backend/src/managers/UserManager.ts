...
export class UserManager {
  // Singletons
  public usersToSocket: Map<string,Socket>;
  public socketIdToUser : Map<string,string>
  private queue: string[];
  private rooms: RoomManager;


  constructor() {
    this.usersToSocket = new Map<string,Socket>();
    this.socketIdToUser = new Map<string,string>();
    this.queue = [];
  ...
  socketHandlers(socket: Socket) {
    socket.on("offer", (data) => {
      console.log(data);
      this.rooms.onNewOffer(data?.offer, data?.roomId, socket.id);
    });

    socket.on("answer", (data) => {
      this.rooms.onAnswer(data?.answer, data?.roomId, socket.id);
    });
  }
}
